...
#     content: Any = Field(..., description="The content of the message, can be text, tool usage, or tool result.")
...
#     validated_history = []
#     try:
#         validated_history = [ConversationMessage(**message).model_dump() for message in conversation_history]
#     except ValidationError as e:
#         raise ValueError(f"Invalid conversation history format") from e

#     function_calls = []
#     tool_results = []

#     if response.stop_reason == "tool_use":
#         tool_calls = [tool_use for tool_use in response.content if isinstance(tool_use, ToolUseBlock)]
        
#         if tool_calls:
#             for tool_call in tool_calls:
#                 if not tool_call.id:
#                     raise ValueError("Tool use block is missing an ID and is unable to resolve the tool call.")
...
#         assistant_message_content.append(block.to_dict())
